  ...
  const handleAddCron = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage({
          type: "error",
          text: data.error || "Failed to add cron job",
        });
      }
    } catch (error) {
          ...
          <div className="max-w-3xl mx-auto space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Configure automatic monitoring using the <code>monitor</code>{" "}
                  command. This will check server health and restart if needed.
                </CardDescription>
              </CardHeader>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder={
                      server.local
                        ? "Current user"
                        : (server.sshConnection as any)?.username || ""
                    }
                    value={username}
                    onChange={(e) => setUsername(e.target.value)}
                  />
                  <p className="text-sm text-muted-foreground">
                    The user that runs the game server. Usually matches the SSH
                    user.
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Add this line to the crontab of the user that will run the
                    monitor.
                  </p>
                ...
                {message && (
                  <div
                    className={`p-3 rounded-md text-sm ${message.type === "success" ? "bg-green-500/15 text-green-500" : "bg-destructive/15 text-destructive"}`}
                  >
                    {message.text}
                  </div>
                )}
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-sm">
                      ...
                      <p className="text-muted-foreground mt-1">
                        The monitor command will automatically restart the
                        server if it crashes or becomes unresponsive. Make sure
                        to configure alert notifications to be notified of
                        restarts.
                      </p>
                    </div>
                  </div>
