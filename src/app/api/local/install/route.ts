...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!game) {
      return NextResponse.json(
        {
          error: "Game not found in catalog",
          availableIds: GAMES_CATALOG.map((g) => g.id),
        },
        { status: 404 },
      );
    }
    ...
    const depsCheck = await executor.execute(
      "which curl && which unzip && which tmux && which git",
    );
    ...
    if (!exists) {
      ...
      const mvRes = await executor.execute(
        `mv LinuxGSM-master/linuxgsm.sh ${scriptPath}`,
      );
      if (!mvRes.success) {
        return NextResponse.json(
          {
