...
export async function discoverServers(
  ...
  const result = await client.execute(
    `find ${basePath} -maxdepth 3 -name "lgsm" -type d 2>/dev/null`,
  );
  ...
  for (const lgsmDir of lgsmDirs) {
    ...
    const scriptResult = await client.execute(
      `ls ${lgsmDir}/../*.sh 2>/dev/null | head -1`,
    );

    if (scriptResult.success && scriptResult.output) {
      const scriptName =
        scriptResult.output.split("/").pop()?.replace(".sh", "") || "";
...
export async function createServerFromDiscovery(
  ...
  const detailsResult = await client.execute(
    `${discovered.path}/${discovered.name} details`,
  );
