...
export class LinuxGSMService {
  ...
  private parseDetails(output: string): ServerDetails {
    ...
    for (const line of lines) {
      ...
      else if (line.includes("Server Details") || line.includes("Server name:"))
      ...
      else if (line.includes("Command-line Parameters"))
        currentSection = "params";

      // Parse each section
      if (currentSection === "distro") {
        if (line.includes("Distro:"))
          details.distro.distro = line.split(":").pop()?.trim() || "";
        if (line.includes("Arch:"))
          details.distro.arch = line.split(":").pop()?.trim() || "";
        if (line.includes("Kernel:"))
          details.distro.kernel = line.split(":").pop()?.trim() || "";
        if (line.includes("Hostname:"))
          details.distro.hostname = line.split(":").pop()?.trim() || "";
        if (line.includes("tmux:"))
          details.distro.tmux = line.split(":").pop()?.trim() || "";
        if (line.includes("GLIBC:"))
          details.distro.glibc = line.split(":").pop()?.trim() || "";
      }

      if (currentSection === "performance") {
        if (line.includes("Uptime:"))
          details.performance.uptime = line
            .split(":")
            .slice(1)
            .join(":")
            .trim();
        if (line.includes("Avg Load:"))
          details.performance.avgLoad =
            line.split(":").pop()?.trim().split(",") || [];
      ...
      if (currentSection === "disk") {
        if (line.includes("Disk available:"))
          details.disk.available = line.split(":").pop()?.trim() || "";
        if (line.includes("Serverfiles:"))
          details.disk.serverfiles = line.split(":").pop()?.trim() || "";
        if (line.includes("Backups:"))
          details.disk.backups = line.split(":").pop()?.trim() || "";
      }
    }
  ...
  async checkUpdate(): Promise<{ available: boolean; output: string }> {
    const result = await this.client.execute(
      `${this.getScriptPath()} check-update`,
    );
    return {
  ...
  async backup(name?: string): Promise<{ success: boolean; output: string }> {
    ...
    const result = await this.client.execute(
      `${this.getScriptPath()} backup ${backupName}`,
    );
    return { success: result.success, output: result.output };
  }
  ...
  async restore(
    ...
    const result = await this.client.execute(
      `${this.getScriptPath()} restore ${backupFile}`,
    );
    return { success: result.success, output: result.output };
  }
