...
export class LocalLinuxGSMService {
    ...
    for (const line of lines) {
      ...
      if (line.includes(":")) {
        ...
        if (currentSection === "distro") details.distro[keyLower] = value;
          ...
          details.performance[keyLower] = value;
  ...
  async backup(
    name?: string,
  ): Promise<CommandResult & { backupFile?: string }> {
    const cmd = name
  ...
  async getConsoleLog(lines: number = 100): Promise<string> {
    const result = await this.executor.execute(
      `tail -n ${lines} "${this.server.installPath}/log/console"`,
    );
  ...
  async checkPorts(): Promise<{ usedPorts: number[]; conflicts: any[] }> {
    ...
    const conflicts = portsToCheck.filter((p) =>
      usedPorts.has(p.port as number),
    );
